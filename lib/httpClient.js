...
function HttpClient(operation, params) {
	...
	self.makeRequest = function () {
		...
		if (typeof params.requestGenerator == 'function') {
			request = params.requestGenerator(params, options, reqLib, getRequestLibCallback(id, requestFinished));
		}
		else {
  ...
  */
	function getRequestLibCallback(id, callback) {
		return function (error, resp, body) {
			if (error) {
				return callback('Connection ' + id + ' failed: ' + error, null)
			}
